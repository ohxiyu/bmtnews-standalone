...
const baseline=process.env.BROWSER_BASELINE==='1';
...
for(const viewport of [{width:390,height:844},{width:1280,height:900}]) {
 ...
 await context.route(origin+'/**',async route=>{
  ...
  if(url.pathname==='/api/quick-posts.json') return route.fulfill({json:{revision:sha,items:[]}});
  if(url.pathname.startsWith('/editions/')) return route.fulfill({json:{items:[{title:{zh:'第一条新闻'}},{title:{zh:'第二条新闻'}}]}});
  if(url.pathname==='/api/admin/posts') {
   ...
   writes++;items=[{...body,type:'quick_post'}];return route.fulfill({json:{sha,message:'已保存；上线核验结果见列表。'}});
  }
  const path=url.pathname==='/s/'?'s/index.html':url.pathname.slice(1);
  try {let content=baseline?execFileSync('git',['show','origin/main:docs/'+path],{encoding:'utf8'}):await readFile(new URL('../docs/'+path,import.meta.url),'utf8');content=content.replace(/^---\n[\s\S]*?\n---\n/,'');
 ...
 if(!baseline){await page.waitForFunction(()=>document.querySelectorAll('#position option').length===3);
 await page.locator('#position').selectOption('1');}
 ...
 assert.match(await page.locator('#entries').innerText(),baseline?/待核实上线/:/未出现在公开流/);
 assert.equal(errors.length,0,errors.join('\n'));
 if(process.env.BROWSER_EVIDENCE_DIR){
  await mkdir(process.env.BROWSER_EVIDENCE_DIR,{recursive:true});
  await page.screenshot({path:process.env.BROWSER_EVIDENCE_DIR+'/'+(baseline?'before-':'')+'quick-post-'+viewport.width+'.png',fullPage:true});
 }
