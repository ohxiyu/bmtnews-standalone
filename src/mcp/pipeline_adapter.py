"""Adapter layer that reuses BMTNews's native Python modules."""
...
@dataclass
class BMTNewsRuntime:
    """Loaded runtime references from a BMTNews codebase."""

    project_path: Path
    ContentItem: Any
    Config: Any
    StorageManager: Any
    BMTNewsOrchestrator: Any
...
def resolve_project_path(explicit: str | None = None) -> Path:
    """Resolve BMTNews repository path by explicit arg/env/common locations."""

    candidates: list[Path] = []
    if explicit:
        candidates.append(Path(explicit).expanduser())

    env_path = os.getenv("BMTNEWS_PATH")
    if env_path:
    ...
    for candidate in candidates:
        ...
        if path in seen:
            continue
        seen.add(path)
        if _is_project_repo(path):
            return path

    checked = ", ".join(str(p.resolve()) for p in candidates)
    raise BMTNewsMcpError(
        code="BMT_PROJECT_NOT_FOUND",
        message="BMTNews repository was not found. Pass project_path or set BMTNEWS_PATH.",
        details={"checked": checked},
    )


def resolve_config_path(project_path: Path, config_path: str | None = None) -> Path:
    """Resolve config path, defaulting to <bmtnews>/data/config.json."""

    if not config_path:
        path = (project_path / "data/config.json").resolve()
    else:
...
def load_runtime(project_path: Path) -> BMTNewsRuntime:
    """Load BMTNews modules dynamically from local repository path."""

    if not _is_project_repo(project_path):
        raise BMTNewsMcpError(
            code="BMT_INVALID_BMTNEWS_PATH",
            message="project_path is not a valid BMTNews repository.",
            details={"project_path": str(project_path)},
        )

    load_dotenv(project_path / ".env", override=False)
    _load_mcp_secrets(project_path, override=False)
    ...
    try:
        ...
        raise BMTNewsMcpError(
            code="BMT_IMPORT_FAILED",
            message="Failed to load BMTNews modules.",
            details={"error": str(exc)},
        ) from exc

    return BMTNewsRuntime(
        project_path=project_path,
        ContentItem=models.ContentItem,
        Config=models.Config,
        StorageManager=storage.StorageManager,
        BMTNewsOrchestrator=orchestrator.BMTNewsOrchestrator,
...
def load_config(runtime: BMTNewsRuntime, config_path: Path) -> Any:
    """Load BMTNews config using native pydantic model."""
    ...
    try:
        ...
        raise BMTNewsMcpError(
            code="BMT_CONFIG_INVALID",
            message="Failed to parse config file.",
            details={"config_path": str(config_path), "error": str(exc)},
        ) from exc


def make_storage(runtime: BMTNewsRuntime, config_path: Path) -> Any:
    """Build BMTNews storage manager bound to config's data directory."""

    data_dir = str(config_path.parent.resolve())
    return runtime.StorageManager(data_dir=data_dir)


def make_orchestrator(runtime: BMTNewsRuntime, config: Any, storage: Any) -> Any:
    """Build native BMTNews orchestrator."""

    return runtime.BMTNewsOrchestrator(config, storage)
...
def items_to_dicts(items: list[Any]) -> list[dict[str, Any]]:
    """Serialize BMTNews ContentItem models."""

    return [item.model_dump(mode="json") for item in items]


def dicts_to_items(runtime: BMTNewsRuntime, payload: list[dict[str, Any]]) -> list[Any]:
...
def _is_project_repo(path: Path) -> bool:
    return (path / "src" / "main.py").exists() and (path / "pyproject.toml").exists()


def _load_mcp_secrets(project_path: Path, override: bool = False) -> None:
    """Load MCP secrets from JSON and inject string environment variables."""

    secrets_path = _resolve_secrets_path(project_path)
    if not secrets_path:
        return
...
def _resolve_secrets_path(project_path: Path) -> Path | None:
    """Resolve secrets config path via env and common locations."""

    explicit = os.getenv("BMTNEWS_MCP_SECRETS_PATH")
    if explicit:
        explicit_path = Path(explicit).expanduser().resolve()
        if explicit_path.exists():
            return explicit_path
        raise BMTNewsMcpError(
            code="BMT_SECRETS_NOT_FOUND",
            message="BMTNEWS_MCP_SECRETS_PATH points to a missing file.",
    ...
    candidates = [
        ...
        project_path / "data" / "mcp.secrets.json",
        project_path / "data" / "mcp-secrets.json",
    ]
    for candidate in candidates:
