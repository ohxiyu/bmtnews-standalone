#!/usr/bin/env bash
# BMTNews daily run + deploy to GitHub Pages
# Usage: ./scripts/daily-run.sh
# Cron:  0 8 * * * /path/to/bmtnews/scripts/daily-run.sh >> /path/to/bmtnews/logs/cron.log 2>&1
...
echo "$LOG_PREFIX Starting BMTNews daily run..."
