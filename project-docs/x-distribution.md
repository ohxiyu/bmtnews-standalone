...
- **180-300 汉字**，2-4 段，段间空一行
- 结构：直接交代核心事实 → 补关键时间、1-2 个数字或机制 → 停在当前状态
...
URL 一律按 23 计。`max_post_chars` 是加权上限，默认 800（300 汉字约合 600，
并为英文名称和数字留出余量），标准账号请改成 280 —— 但标准账号发不了这类完整简报，那种情况应该同时把
`compose` 改成 `template`。

产出低于 **300 加权单位**（约 150 汉字）会被判为生成失败并回落到模板。
...
- `max_post_chars`：加权字数上限，默认 800；标准账号设 280 并把 `compose` 改成 `template`
