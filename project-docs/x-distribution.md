...
| | `digest` | `drip`（当前配置） |
|---|---|---|
| 时机 | 日报发布完成后 | 日报发布后立即发第 1 条，后续按时段分开发 |
...
`mode: "drip"` 时，日报在 Pages 部署成功后调用一次 `kickoff_only` 任务。
它只会为尚未启动的当期队列发布第 1 条；日报重跑、触发重试或人工重复触发
都不会推进已启动队列。后续定时任务仍按最小未发序号继续，因此日报即使延迟，
首条 X 也会在日报可用后立即补上，而不必等下一档 cron。

## 分时时段

`x-distribution.yml` 的五个 cron（UTC 书写，注释标注东八区）：
...
要改时间直接改 cron；当前文件统一写 UTC。同时 Actions 的定时任务有几分钟到
几十分钟的排队延迟，**而且可能整档丢失**，
...
普通手动运行会消耗一条队列序号，测试时注意。勾选 `kickoff_only` 时，
只有尚未开始的当期队列会发第 1 条，已启动队列不会继续前进。
