...
GitHub 自带的定时任务在本仓库极不可靠（常晚数小时甚至不触发），所以
`x-distribution.yml` 的主时钟是**币安广场 workflow 的完成事件**：Cloudflare
调度 Worker 每 5 分钟（东八区 09:00–23:30）派发一次广场任务，每次完成都会触发
一次 X。广场的 `workflow_run` 触发列表**不能**再包含 X，否则两者互相唤醒。
另保留每 20 分钟一次的 cron（`7-59/20 23,0-15 * * *`）作为兜底。

每次先用系统 Python 跑 `python3 -m src.x_queue` 这个只依赖标准库的门控：
只有新日报/日报重发、有任务到期、有截止时间已过、或有中断的 `pending` 时才安装依赖并执行，
否则几秒内结束。
